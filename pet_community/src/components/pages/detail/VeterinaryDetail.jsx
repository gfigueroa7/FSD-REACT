...
function VeterinaryDetail() {
    ...
    useEffect(() => {
        fetch(`http://localhost:4000/${params.url}_detail/${params.id}`)
          .then((data) => data.json())
          .then(data => {
    ...
    return (
        <>
            <Header mainName="Veterinaria" subName={veterinaryObject.title} btnName="" />
            <section className="section-content">
                ...
                { veterinaryObject.length === 0 ? '' :
                  <WholeDetail object={veterinaryObject} pathname={location.pathname} />    
                }
